...
sql.query(`SELECT tss.candidate_induction_info.*, tss.candidate_induction_emp.*,tss.candidate_induction_gap.*,tss.candidate_induction_language.*,tss.candidate_induction_bank.* FROM tss.candidate_induction_info
...
where tss.candidate_induction_info.id  = ${id}`, async (err, res) => {
 if (err) {
    await  result(err, null);
      ...
      record = res
      // console.log(record)
        sql.query(`SELECT tss.candidate_induction_edu.* FROM tss.candidate_induction_info
        LEFT JOIN tss.candidate_induction_edu 
        ON (tss.candidate_induction_info.id = tss.candidate_induction_edu.id)
        where tss.candidate_induction_info.id  = ${id}`, async (err, res) => {
          if(err) {
            ...
            record.push(res[0])
            await  result(null, record);
          }
        })
      // tss.candidate_induction_edu.*,
    }
  });
};
